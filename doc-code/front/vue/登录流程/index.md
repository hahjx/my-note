...
- 这几个接口的具体使用位置是哪里？
  1. 验证码&登录接口（token获取接口），在登录页调用。
  2. 获取用户信息接口，在路由守卫（permission.js）中调用。
  3. 获取权限与角色接口，在路由守卫中调用。
  4. 获取动态路由/菜单接口，在路由守卫中调用。

- 衍生问题：路由守卫的逻辑是什么？
  - 首先这两个分两种情况：一个是前端写死路由，根据后端给的用户信息用role来筛选，另一种是后端返回动态路由。我们拿动态路由来举例此处的逻辑：
  1. 白名单判断（是否允许直接进入）
  2. token判断(是否登录)
  3. 用户信息/权限初始化判断(是否需要拉取数据)
